function plotsInterspeech16_2(toDisk, startAt, allLabels)
...
if allLabels
    outDir = '~/work/talks/cunynlp-2016-05-13/interspeech16_pics/allLabels/';
else
    outDir = '~/work/talks/cunynlp-2016-05-13/interspeech16_pics/compact/';
end
...
plotOnSpecs(inDir, 'gridAdaptAll', 'human', [-99 4], ...
    @(x) reMatch(x, 'bwir6a'), ...
    @(x) reMatch(x, 'bbikza|bgil8a'), ...
    @(x) reMatch(x, 'b(g|w)i'), allLabels);
